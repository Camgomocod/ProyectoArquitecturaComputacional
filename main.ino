...
String correctPassword = "1234AB";       /**< Contraseña correcta */
...
void keepBuzzer()
{
  if (millis() - lastTimeBuzzer >= 200)
  { // Cambiar intervalo a 200 ms
    lastTimeBuzzer = millis();
    static int pattern[] = {1000, 1200, 1400, 1600}; // Patrón de frecuencias
    static int index = 0;
    tone(BUZZER_PIN, pattern[index]);
    index = (index + 1) % 4; // Ciclo a través del patrón
  }
}
...
void blockade()
{
  ...
  unsigned long timeLeft = 7; // Tiempo de bloqueo en segundos

  while (millis() - startTime < timeLeft * 1000)
  {
    keepBuzzer();
    // Actualiza el tiempo restante en el LCD
    lcd.setCursor(0, 0);
    lcd.print("BLOQUEADO: ");
    lcd.print(timeLeft - (millis() - startTime) / 1000); // Muestra el tiempo restante
    lcd.print("s");

    delay(1000); // Actualiza cada segundo
  }
  // Cuando el tiempo se acabe, activa la transición al estado INICIO
  input = INPUT_T;
  Serial.println("Tiempo de bloqueo terminado. Regresando al estado INICIO.");
}
...
void readKeyboard()
{
  ...
  if (key)
  {
    ...
    if (stateMachine.GetState() == INICIO)
    {
      ...
      {
        inputPassword += key;                         // Guarda el carácter real en la variable
        lcd.setCursor(inputPassword.length() - 1, 1); // Mover el cursor hacia la derecha
        lcd.print("*");                               // Mostrar un asterisco en lugar del carácter real
      }
    }
  }
}
